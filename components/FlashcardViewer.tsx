...
// Exporting the interface so your data files and pages know how to structure the cards
...
export default function FlashcardViewer({ cardsData, returnHref, moduleTitle, storageKey }: FlashcardViewerProps) {
  ...
  // Prevent Next.js hydration errors and load saved "Hard" cards
  useEffect(() => {
    setIsMounted(true);
    const savedHardCards = localStorage.getItem(storageKey);
    if (savedHardCards) {
  ...
  const startReviewHard = () => {
    const reviewDeck = cardsData.filter(card => hardCards.includes(card.id));
  ...
  const handleRating = (rating: string) => {
    ...
    localStorage.setItem(storageKey, JSON.stringify(updatedHardCards));
